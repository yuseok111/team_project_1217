...
var connection = mysql.createConnection({
  host: "database-2.c8v9aygdaryr.ap-northeast-1.rds.amazonaws.com",
  user: "root",
  password: "525252",
  database: "team_bogo",
  dateStrings: "date",
...
function insertNoticeWrite(noticefile, noticetitle, noticecont, callback) {
  connection.query(
    `insert into notice(DATE, noticefile, noticetitle, noticecont)
  values(now(),'${noticefile}','${noticetitle}','${noticecont}')`,
    (err) => {
      if (err) throw err;
      callback();
    }
...
function updateNoticeWrite(noticefile, noticetitle, noticecont, id, callback) {
  connection.query(
    `update notice set DATE=now(),noticefile='${noticefile}',noticetitle='${noticetitle}',noticecont='${noticecont}' where id=${id}`,
    (err) => {
      if (err) throw err;
      callback();
    }
...
function getnoticecount(id, callback) {
  connection.query(
    `update notice set noticecount = noticecount+1 where id=${id}`,
    (err, rows, fields) => {
      if (err) throw err;
      callback(rows);
    }
...
function getnxtnotice(id, callback) {
  connection.query(
    `SELECT id FROM notice WHERE id IN(SELECT * from(SELECT id FROM notice WHERE id > ${id}  ORDER BY id LIMIT 1)as t)`,
    (err, row, fields) => {
      if (err) throw err;
      callback(row);
      // console.log(rows);
...
function insertcollection(img, name, writer, year, cont, callback) {
  connection.query(
    `INSERT INTO bogo_collection(create_time,img,title,writer,collection_year,collection_cont) value (NOW(),'${img}','${name}','${writer}','${year}','${cont}')`,
    (err) => {
      if (err) throw err;
      callback();
    }
...
function getcollectionByid(id, callback) {
  connection.query(
    `SELECT * FROM bogo_collection WHERE id = ${id}`,
    (err, row) => {
      if (err) throw err;
      callback(row);
    }
...
function updateprowrite(img, id, name, writer, year, cont, callback) {
  connection.query(
    `UPDATE bogo_collection SET create_time= NOW(),img='${img}',title='${name}',writer='${writer}',collection_year='${year}',collection_cont='${cont}' WHERE id =${id}`,
    (err) => {
      if (err) throw err;
      callback();
    }
